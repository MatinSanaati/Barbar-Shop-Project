...
const CalendarModal = ({ onSelectDate, onClose }) => {
    ...
    const [isVisible, setIsVisible] = useState(true); // ✅ برای انیمیشن بسته شدن
    ...
    // روزهای هر ماه (ساده شده)
    const daysInMonth = (month, year) => {
        if (month <= 6) return 31;
        if (month <= 11) return 30;
        return (year % 4 === 3) ? 30 : 29; // اسفند
    };
    ...
    // ✅ بستن با انیمیشن
    const handleClose = () => {
        setIsVisible(false); // اول انیمیشن بسته شدن
        setTimeout(() => onClose(), 300); // بعد از 0.3 ثانیه، onClose فراخوانی شه
    };
    ...
    const handleSelect = () => {
        ...
        handleClose(); // استفاده از handleClose به جای onClose مستقیم
    };
